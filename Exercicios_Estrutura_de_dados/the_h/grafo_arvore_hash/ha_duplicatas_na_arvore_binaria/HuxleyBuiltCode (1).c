...
int aux(int item, ARVORE_BINARIA *arvore, int cont){
    
    if(arvore){
        
        if(item == arvore->item.chave)cont++;

        if(cont >= 2)return 1;
        
        else{
            return aux(item, arvore->esq, cont);
            return aux(item, arvore->dir, cont);
        }
    }
    else{
        return 0;
...
int percorrer_arv(ARVORE_BINARIA *arvore, ARVORE_BINARIA *raiz){
    if(arvore){
        if(aux(arvore->item.chave, raiz, 0) == 1)return 1;

        else{
            return percorrer_arv(arvore->esq, raiz);
            return percorrer_arv(arvore->dir, raiz);
        }
    }
    else{
        return 0; 
...
if(percorrer_arv(arvore, raiz))return 1;
    else return 0;
