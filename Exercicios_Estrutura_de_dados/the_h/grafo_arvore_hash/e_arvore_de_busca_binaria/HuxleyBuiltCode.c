...
if(arvore == NULL)return true;
        ...
        if(arvore->dir == NULL && arvore->esq == NULL && arvore->pai == NULL)return true;
        
        else{
            if(arvore->esq){
                if(arvore->esq->item.chave > arvore->item.chave)return false;
                else if(arvore->esq->item.chave == arvore->item.chave)return false;
                else return eBinariaBusca(arvore->esq);
            }
            if(arvore->dir){
                if(arvore->dir->item.chave < arvore->item.chave)return false;
                else if(arvore->dir->item.chave == arvore->item.chave)return false;
                else return eBinariaBusca(arvore->dir);
            }
        }
    }
    return true;
